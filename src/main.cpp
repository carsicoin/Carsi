...
uint256 hashGenesisBlock("0x0cc4eaa3093f45de8127ca49151315a7b4a3581260386e4c80aaaae6b75ee706");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CarsiCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CarsiCoin Signed Message:\n";
...
{
    // CarsiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 19030000 * 9  / 100 / 15;
     }

    nSubsidy >>= (nHeight / 577244);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CarsiCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CarsiCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CarsiCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0cc4eaa3093f45de8127ca49151315a7b4a3581260386e4c80aaaae6b75ee706");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1420661858;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086313785;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CarsiCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CarsiCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running CarsiCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CarsiCoinMiner terminated\n");
        throw;
    }
}
